...
const AnimeEpisode: React.FC<AnimeEpisodeProps> = memo(
    ({ detail, className, onSelectVideo }) => {
        ...
        return (
            <div
                className={cn(
                    'bg-card dark:bg-muted rounded-sm overflow-hidden',
                    className
                )}
            >
                <div
                ...
                <div
                    ref={containerRef}
                    className={cn(
                        'max-h-[13.75rem] overflow-auto outline-none'
                    )}
                >
                    {list.map(item => {
...
AnimeEpisode.displayName = 'AnimeEpisode';
