import { cn } from '@/lib/utils';

const Logo = ({ size, className }: { size: string; className?: string }) => {
    return (
        <svg
            className={cn(className)}
            xmlns="http://www.w3.org/2000/svg"
            width={size}
            height={size}
            viewBox="0 0 113.92000579833984 100"
        >
            <g fillRule="evenodd">
                <path
                    d="M57.21 88c12.71 6.9 23.09 10.57 30.17 10.61a1.85 1.85 0 0 1 .23 0s.06 0 .06-.08c.63-2.76.67-6.63.22-11.56a159.45 159.45 0 0 0-3.84-21.82A207 207 0 0 1 57.21 88z"
                    fill="#fb7299"
                ></path>
                <path
                    d="M92.63 31.7C92.12 19.4 90.25 9.58 87.14 3c-5.79 2.4-14 8.17-24.43 17a192.31 192.31 0 0 1 29.92 11.7z"
                    fill="#23ade5"
                ></path>
                <path
                    d="M28.67 30.78C14.81 38 4.93 45.12 0 51.41c4.49 4.44 12.63 9.2 24 14.18 0-.88.06-1.75.07-2.65s.07-1.84.11-2.78c.84.15 1.69.27 2.55.43q3.77.54 7.8 1a265.86 265.86 0 0 1-5.86-30.81z"
                    fill="#fbbc09"
                ></path>
                <path
                    d="M26.58 63.44a156.32 156.32 0 0 0 .68 22.77 56.8 56.8 0 0 0 2.95 13.57.66.66 0 0 0 .15.22h.11c5.8-1.58 13.4-5.68 22.46-12.06a231.19 231.19 0 0 0 32.2-27.74c1.37 3.49 2.57 6.84 3.67 10 12.8-7.69 21.48-15.1 25.12-21.45-3.06-3.91-9.12-8.18-17.83-12.71a207.27 207.27 0 0 0-38.4-14.74q3.66-4.18 7.06-7.79C51.14 5.38 39.23.71 30.51 0c-1.11 5.08-1 12.88.08 23.25a291.3 291.3 0 0 0 7.79 41.88q-6.18-.76-11.8-1.69z"
                    fill="#615fff"
                ></path>
            </g>
        </svg>
    );
};

export default Logo;
